...
patchwork.step
==============
Base class and status enum for all NullShift steps.

A **Step** is an atomic, reusable unit of work.  Every step:
- Declares its required inputs via a ``TypedDict`` (``input_class``).
- Declares its outputs via a ``TypedDict`` (``output_class``).
- Implements a single ``run()`` method that returns a dict of outputs.
...
...     result: str
>>>
>>> class UpperCase(Step, input_class=MyInputs, output_class=MyOutputs):
...     def run(self):
